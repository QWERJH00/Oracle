...
create sequence 시퀀스명
    [imcrement by N] => 증가치 설정
    [Start with N] => 시작값 지정
    [Mimvalue n | NoMinvalue] => 시퀀스 최소값 지정 : 디폴트 1
    [Maxvalue n | NoMexvalue] => 시퀀스 최대값 지정 : 디폴트 1.0000E+28
    [Cycle | No Cycle] => 최대/최소값에 도달할 경우 처음부터 다시
                          시작할지 여부를 설정(Cycle로 지정하면 최대값까지
                          증가후 다시 시작값부터 재시작된다.
    [Cache | No Cache] => cache 메모리에 오라클 서버가 시퀀스값을
                          할당하는 여부를 지정
주의사항
    1.start with 에 minvalue보다 작은값을 지정할 수 없다. 즉 start wwith값은
    minvalue와 같거나 커야한다.
...
insert into tb_goods values(1, '가나초콜렛'); 
...
/*
시퀀스 생성 후 최초실행시 오류가 발생. nextval을 먼저 실행한 후
실행해야 문제없이 출력된다.
*/
...
인덱스(Index)
    ...
    - 인덱스는 명시적(create index)과 자동적(primary key, unique)으로 생성가능
...
/*
특정 테이블의 인덱스를 확인
    : 데이터사전에 등록시 대문자로 입력되므로 upper와 같은 변환함수를
    사용하면 편리하다.
*/    
...
-- 인덱스 수정 : 수정은 불가능하다. 삭제후 다시 생성해야한다.
