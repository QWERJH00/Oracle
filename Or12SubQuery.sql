...
/*
복수행 연산자 : any (any의 개념은 or와 비슷하다.)
    메일쿼리의 비교조건이 서브쿼리의 검색결과와 하나이상
...
시나리오) 전체 사원중에서 부서번호가 20인 사원들의 급여보다 높은 급여를
    받는 직원들을 인출하는 서브쿼리문을 작성하시오. 단, 둘중 하나만 만족
    하더라도 인출하시오.
*/
-- 1단계 : 20번 부서부터 급여를 확인한다.
...
/*
복수행연산자3 : all (all은 and의 개념과 유사하다)
    메인쿼리의 비교조건이 서브쿼리의 검색결과와 모두 일치해야
    레코드를 인출한다.
*/

/*
시나리오) 전체 사워중에서 부서번호가 20인 사원들의 급여보다 높은급여를
...
-- 이름에 오름차순으로 정렬하면 rownum이 섞여서 이상하게 나온다.
select employee_id, first_name, rownum from employees
order by first_name;

/*
rownum을 우리가 정렬한 순서대로 재부여하기 위해 서브쿼리를 사용한다.
from절에는 테이블이 들어와야 하는데 아래에 서브쿼리에서는 사원테이블의
