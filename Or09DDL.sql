...
*/
-- oracle21c 이상부터는 계정 생성전 해당명령을 실행해야한다.
...
create table tb_member (
    idx number(10), -- 10자리의 정수를 포현
    userid varchar2(30), -- 문자형으로 30byte 저장가능
    passwd varchar2(50), -- 50바이트
    username varchar2(30), -- 30바이트
    mileage number(7,2) -- 실수표현, 전체7자리, 소수이하 2자리 표현.
    );
...
-- 삭제후 테이블 목록에서는 보이지 않는다. 휴지통에 들어간 상태.
select * from tab;
-- 객체가 존재하지 않습니다라는 오류가 발생한다.
desc employees;
...
/*
오라클 11g에서는 새로운 계정을 생성한 후 connect, resource 를 (Role)만
부여하면 테이블 생성 및 삽입까지 되지만, 그 이후버전에서는 테이블스페이스
관련 오류가 발생한다. 따라서 아래와 같이 테이블 스페이스에 대한 권한도
부여해야한다.
해당 명령은 system계정으로 접속한 후 실행해야한다.
*/
...
(1, 'hong', '1234','홍길동', 'hong@naver.com');
insert into tb_member values 
(1, 'yu', '9876','유비', 'yoo@hanmail.com');
--삽입된 레코드를 확인한다.
select * from tb_member;

-- 테이블 복사하기1 : 레코드까지 함께 복사
/*
    select문을 기술할때 where절이 없으면 모든 레코드를 출력하라는
