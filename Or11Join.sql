...
1) inner join(내부조인)
    ...
    - 두개의 테이블에 동일한 이름의 컬럼이 존재하면 "테이블명.컬럼명"
      형태로 기술해야 한다.
    - 테이블에 별칭을 사용하면 "별칭.컬럼명"형태로 기술할 수 있다.  
...
/*
시나리오) 사원테이블과 부서테이블을 조인하여 각 직원이 어떤부성에서
...
00918.00000 - "column ambigguously defined"
department_id는 조인한 양쪽 테이블에 존재하는 컬럼이므로
어떤 테이블에서 가져와 출력할지 결정해야한다.
*/
select
    employees_id, first_name, last_name, email, department_id,
    department_name
from
...
인출된다. 즉 inner join 은 조인한 테이블에서 양쪽 모두 만족하는 레코드만 
가져오게 된다.
*/

--3개 이상의 테이블을 조인하기
/*
시나리오) seattle(시에틀)에 위치한 부서에서 근무하는 직원의 정보를
...
-- 5. join 쿼리문 작성
select first_name, last_name, email, D.department_id, department_name,
...
형식2) 오라클방식
    ...
    from 테이블1, 테이블2,...
    where
        테이블1.기본키컬러 = 테이블2.외래키컬럼
        and 조건1 and 조건2...;
*/

/*
시나리오) 사원테이블과 부서텡이블을 조인하여 각 직원이 어떤부서에서
...
담당 업무명, 근무지역
위 출력결과는 다음 테이블에 존재한다.(위에거랑 조건 동일)
사원테이블 : 사원이름, 이메일, 부서아이디, 담당업무 아이디
        부서테이블 : 부서아이디(참조), 부서명, 지역일련번호(참조)
        담당업부테이블 : 담당업무명, 담당업무아이디(참조)
        지역테이블 : 근무부서, 지역일련번호(참조)
*/
select first_name, last_name, email, D.department_id,
...
2) outer join (외부조인)
    ...
    쿼리문을 작성해야한다.
    => left(왼쪽 테이블), right(오른쪽 테이블), full(양쪽텡이블)
...
=> 기준이 되는 테이블을 변경할 때는 테이블에 위치를 옮겨준다. (+)를
    옮기지 않는다.
*/
...
/*
시나리오)연습문제) 2007년에 입사한 사원을 조회하시오. 
단, 부서에 배치되지 않은직원의 경우 <부서없음>으로 출력하시오.
단, 오라클 방식으로 작성하시오.
출력항목 : 사번, 이름, 성, 부서명
*/
select
...
/*
3) self join(셀프조인)
    셀프 조인은 하나의 테이블에 있는 컬럼끼리 연결해야 하는 경우 사용.
...
/*
시나리오) 사원테이블에서 각 사원의 매니저아이디와 메니저이름을 출력하시오.
    단, 이름은 first_name과 last_name을 하나로 연결해서 출력하시오.
*/
/*
여기서는 사원입장의 테이블 empClerk과 매니저입장의 테이블 empManager를
...
select
 ...
 where  Kimberely.hire_date < Clerk.hire_date
...
-- 4. join쿼리문 작성하기
select employee_id, first_name, last_name, trim(to_char(salary, '$990,000'))"급여",
