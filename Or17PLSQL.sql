...
PL/SQL(procedural language)
    ...
    DB업무를 처리하기 위한 최적화된 언어이다.
*/

-- 예제1)
-- 화면상에 내용을 출려하고 싶을때 on으로 설정. off일때는 출력x
set serveroutput on;

declare -- 선언부 :주로 변수를 선언
    cnt number; -- 숫자탕비의 변수를 선언
begin -- 실행부
    cnt := 10;
    cnt := cnt +1;
    dbms_output.put_line(cnt); -- 자바의 println()과 동일
end;
/
...
-- 예제3) 참조변수1 (하나의 컬럼 참조)
/*
    참조변수 : 특정 테이블의 컬럼을 참조하는 변수로서 동일한 자료형과
        크기로 선언하고 싶을때 사용한다.
        형식) 테이블명.컬럼명%type
            => 테이블의 '하나'의 컬럼만 참조한다.
*/
...
-- 예제4) 참조변수2 (전체칼럼을 참조)
/*
시나리오) 사원번호가 100인 사원의 레코드를 가져와서 emp_row변수에 전체컬럼
    을 저장한 후 화면에 다음 정보를 출력하시오.
    단, emp_low는 사원 테이블이 전체컬럼의 저장할 수 있는 참조변수로 선언해
    야 한다.
    출력정보 : 사원번호, 이름, 이메일, 급여
*/
declare
    /*
    사원 테이블 전체 컬럼을 참조하는 참조변수로 선언한다.이때 테이블명
    뒤에 %rowtype을 붙여 선언한다.(*와 비슷)
    */
...
복합변수
    ...
    복합변수 자료형을 만들때는 일반변수화 참조변수 2가지를 복합해서
    사용할 수 있다.
*/
...
위에서 생성한 자료형을 이용하여 복합변수 rec2를 생성후 사원번호 145번의 정보를 할당한다.
2.1의 내용을 출력한다.
...
-- PL/SQL 작성
declare
    -- 선언부에는 이와같이 아무내용 없을수도 있다.
begin
    -- 바인드 변수는 일반변수와의 구분을 위해 :(콜론)을 추가해야한다.
    :return_var :=999;
    dbms_output.put_line(:return_var);
end;
/
-- 호스트환경에서 출력시에는 print를 사용한다.
-- 만약 출력이 안된다면 CMD에서 확인해보며녀 정상적으로 출력된다.
print return_var;
...
/*
반복문1 : Basic loop
    java do~while문과 같이 조건체크없이 loop진입한 후 탈출조건이
...
begin
    ...
    loop
        ...
        -- 증가연산자 복합대입연산자가 없으므로 일반적인 방식을로
        -- 변수를 증가시켜야 한다.
        num := num +1;
        -- num1 10을 초과하면 loop문을 탈출한다.
        -- esitsms java의b break;와 동일한 역할을 한다.
        exit when (num>10);
    end loop;
end;
/
...
declare
    ...
    -- 누적합을 저장할 변수
    sumNum number := 0;
    -- 변수명으로 sum을 사용할 수 없다. 예약어(그룹함수)이므로 오류가 발생
begin
    loop
        -- 증가하는 변수 i를 누적해서 더한다.
        sumNum := sumNum + i;
...
/*
반복문2 :  while문
    Vasic loop와는 다르게 조건을 먼저 확인한 후 실행한다.
...
예제15] 제어문(반복문 : for)
    : 반복의 횟수를 지정하여 사용할 수 있는 반복문으로, 반복을 위한 변수를
        별도로 선언하지 않아도 된다. 그로므로 특별한 이유가 없으면
...
begin
    ...
    for num2 in 0 .. 10 loop
        dbms_output.put_line('for문장인듯:' || num2);
        end loop;
    end;
    /
-- 필요없는 경우라면 declaer절은 생략할 수 있다.
begin
    for num3 in reverse 0 .. 10 loop
        dbms_output.put_line('거꾸로 for문장인듯:' || num3);
        end loop;
    end;
    /
...
커서(Cursor)
    ...
    선언방법)
        ...
        Open Cursor
            ...
            실행되어 결과셋을 얻게된다. Cursor는 그 결과셋의 첫번째 행에
            위치하게 된다.
        fetch~Into~
        ...
        Cursor의 속성
            ...
            False를 반환한다.
            %NotFound : %의 반대의 값을 반환한다.
            %RowCount : 지금까지 Return된 행의 개수를 반환한다.
*/

-- 예제17] Cursor
/*
시나리오] 부서테이블의 레코드를 Cursor를 통해 출력하는
PL/SQL문을 작성하시오.
*/
declare
...
begin
    /*
     해당 퀴리문을 수행해서 결과셋(ResultSet)을 가져온다. 결과셋이란
...
1.연관배열(Associative Array)
    ...
    Key : 자료형은 주로 숫자를 사용한다. binary_infefrt, pls_integer를 주로
    사용하는데 number타입보다 크기는 작지만, 산술연산에 빠른 특징을 가진다.
...
begin
    /*
    커서를 오픈하여 쿼리문을 실행한 후 얻어온 결과셋의 개수만큼 반복하여
    사원명을 인출한다.
    */
    open emp_cur;
    loop
        fetch emp_cur into fname;
        -- 더이상 인출할 내용이 없다면 loof를 탈출한다.
    ...
    close emp_cur;
    -- 연관배열.count : 연관배열에 저장된 원소의 개수를 반환한다.
...
예제21] VArray(Variable Array)
    : 고정길이를 가진 베열로서 일반 프로그래밍 언어에서 사용하는 배열과
    동일하다. 크기에 제한이 있어서 선언할 때 크기(원소의 개수)를 지정하면
    이보다 큰 배열로 만들 수 없다.
    형식) Type 배열타입명 Is
            Array(배열크기) of 값의 타입;
*/
declare
    ...
    cnt number := 0;
begin
    -- 생성자를 통한 값의 초기화. 총 5 개중 3개만 할당한다.
...
3. 중첩 테이블(Nested table)
    ...
    종적으로 배열의 크기가 설정된다. 여기서 말하는 테이블은 잘료가 저장되는
...
begin
    ...
    --nt_array(5) := '다섯번째값?할당?';
        ...
        -- 크기를 확장했을때 생성자를 통해 별의 크기를  동적으로 확장한다.
        -- 크기가 7인 중첩테이블로 확장된다.
...
declare
    ...
    */
    ...
    nt_array ntType;
begin
    -- 크기를 지정하지 않는 상태로 생성자를 통해 중첩테이블을 초기화한다.
    nt_array := ntType();
