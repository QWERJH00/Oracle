...
퀴즈) 위에서 생성한 view_employees 뷰를 아래 조건에 맞게 수정하시오
    직무아이디 ST_MAN인 사원의 사원번호, 이름, 이메일, 매니저아이디를
    조회할 수 있도록 수정하시오.
...
퀴즈) 사원번호, 이름, 연봉을 계산하여 출력하는 뷰를 생성하시오.
    컬럼의 이름은 emp_id, l_name, annul_sel로 지정하시오.
    연봉계산식 => (급여 + (급여*보너스율))*12
    뷰이름 : v_emp_salary
...
create or replace view v_emp_salary(emp_id, l_name, annul_sel)
as
select
    employee_id, last_name, 
    rtrim(to_char((salary+(salary*nvl(commission_pct,0)))*12,'$999,000')) "연봉"
...
/*
join을 통한 view 생성
시나리오) 사원테이블과 부서테이블, 지역 테이블을 조인하여 다음 조건에 맞는
뷰를 생성하시오
