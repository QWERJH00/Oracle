...
제약 조건 및 테이블 목록 확인하기
    ...
    user_cons_columns : 테이블에 지정된 제약조건과 컬럼명의 간략한
    정보를 저장한다.
    user_constaints : 테이블에 지정된 제약조건에 보다 사아세한 정보를
    저장한다.
...
insert into tb_primary1 (idx, user_id, user_name)
    values (2, 'seoulit1', '오류발생');
/*
무결성 제약 조건 위배로 에러가 발생. primary key로 지정된 컬럼 idx에는
중복된 값을 입력할 수 없다.
*/
insert into tb_primary1 (idx, user_id, user_name)
    values (3, 'white', '화이트');
insert into tb_primary1 (idx, user_id, user_name)
    values ('', 'black', '블랙');
-- PK(프라이머리 키)로 지정된 컬럼에는 null값을 입력할 수 없다.
select * from tb_primary1;

update tb_primary1 set idx=4 where user_name='유산슬';
/*
update문은 정상이지만 idx값이 이미 존재하는 2로 변경했으므로
제약 조건 위배로 오류가 발생한다.(2는 이미 있는데 바꾸려해서,추가할라면
4이상으로)
*/
...
alter table 테이블명 add [constraints 제약명] primary key (컬럼명);
*/
...
-- 데이터 사전에서 제약조건 확인하기
select * from user_constraints;
-- 제약조건은 테이블을 대상으로 함으로 테이블이 삭제되면 같이 삭제된다.

drop table tb_primary3;
-- 확인시 휴지통에 들어간 것을 볼수있다.
select * from user_cons_columns;
...
/*
    ...
    컬럼이 동시에 동일 값을 가지는 경우가 아니라면 중복된 값이 허용된다.
    즉4,5번은 서로 다른 데이터로 인식하므로 입력되고,4번과6번은
...
create table tb_foreign1 (
    ...
    f_name varchar2(50),
    /*
        자식 테이블인 tb_foreign1에서 부모 테이블인 tb_primary2의 user_id
        컬럼을 참조하는 외래키를 생성한다.
    */
...
/*
    ...
    외래키를 생성했다면 레코드는 삭제되지 않고 오류가 발생한다.
*/
...
/*
    on delete set null 옵션으로 자식테이블의 레코드는 삭제되지않고 참조키
    부분만 null값으로 변경된다. 따라서 더이상 참조할 수 없는 레코드로 변경.
*/
...
create table tb_not_null (
    ...
    m_name varchar2(40) --  null 허용(이와같이 사용(위에처럼 안하면 디폴트임))
    );
desc tb_not_null;
-- 10~30까지는 정상적으로 입력된다.
...
-- m_id는 not null로 지정되었으므로 null값을 삽입할 수 없어 오류발생
...
/*
check : Domain(자료형 무겨성을 유지하기 위한 제약조건으로 해당 컬럼에
...
insert into tb_check values ('M');
insert into tb_check values ('F');
-- check 제약조건 위배로 오류발생
insert into tb_check values ('T');
-- 입력된 데이터가 컬럼의 크기보다 크므로 오류발생
insert into tb_check values ('여성');

-- 10이하의 값만 입력할 수 있는 check 제약조건지정
create table tb_check2 (
    sale_conut number not null,
    check (sale_conut<=10)
    );
