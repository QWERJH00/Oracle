...
/*
서브프로그램(Sub Program)
    -PL/SQL에서는 프로시저와 함수라는 두자리 유형으 서브프로그램이 있다.
    -Select를 포함해서 다른 DML문을 이용하여 프로그래밍적인 요소를 통해
    사용가능하다.
    - 트리거는 프로시저의 일종으로 특정 테이블의 레코드의 변화가 있을경우
    자동으로 실행된다.
    - 함수는 쿼리문의 일부분으로 사용하기 위해 생성한다. 즉 외부 프로그램
    에서 호출하는 경우는 거의 없다.
    - 프로시저는 외부프로그램에서 호출하기 위해 생성한다. 따라서 java,jsp
...
1.저장 프로시저(Stored Procedure)
    ...
    형식) create [or replace] procedure 프로시저명ㅇ
...
is
    ...
    변수를 생성. 만약 변수가 필요없다면 생략가능
    사원테이블의 급여 컬럼을 참조하는 참조변수 생성
    */
    v_salary employees.salary%type;
begin
    --100번 사원의 급여를 into를 이용해서 변수에 할당
    ...
    /
-- 데이터 사전에서 확인. 저장시 대문자로 변환되므로 변환함수를 는게 좋음
...
*/
-- 프로시져 생성시 in파리미터를 설정한다. 사원테이블의 사원명 컬럼을 참조하는
-- 참조변수로 생성한다.
create procedure pcd_in_param_salary
    (param_name in employees.first_name%type)
is
    /*
    변수는 is절에 선언하고 , 필요없는 경우 선언하지 않을수도 있다.
    */
    valSalary number(10);
begin
    /*
    인파라미터로 전달된 사원명을 조건으로 급여를 구한 후 변수에 할당한다.
    하나의 겱과가 출력되므로 into를 select절에서 사용할 수 있다.
    */
...
필요하지 않아 is 절을 비워둔다. 이와같이 변수선언은 생략할 수 있다.
*/
begin
    /*
    ...
    */
    ...
    /
-- 호스트 환경에서 바인드 변수를 선언한다. var 또는 variable둘다 사용할 
-- 수 있다.
var v_salary varchar2(30);
/*
프로시저 호출시 각각의 파라미터를 전달한다. 특히 바인드 변수는 :을 붙여야
한다. Out파라미터인 param_salary에 저장된 값이 v_salary로 전달된다.
*/
execute pcd_out_param_salary('Matthew', :v_salary);
-- 프로시저 실행후 out파라미털를 통해 전달된 값을 출력한다.
print v_salary;
...
*/
-- 실습을 위해 impolyees테이블을 레코드까지 복사한다.
create table zcopy_employees
    as
    select * from employees;
-- 복사되었는지 확인한다.
select * from zcopy_employees;
-- in 파라미터는 사원번호와 급여를 전달한다. out파라미터는 적용된 행의
-- 갯수를 반환받는다.
...
is
-- 추가적인 변수선언이 필요없으므로 생략한다.
begin
    -- 실제 업데이트를 처리하는 쿼리문으로 in 파라미터를 통해 값을 설정
...
2. 함수
    ...
    정의한 것이다.
    - 함수는 In파라미터만 사용할 수 있고, 반드시 반환된 값의 자료형을 명시
    해야한다.
    - 프로시저는 여러개의 결과값을 얻어올 수 있지만, 함수는 반드시 하나의
    값을 반환해야한다.
    - 함수는 쿼리문의 일부분으로 사용된다.
    * 파라미터의 반환탕임을 명시할때 크기는 기술하지 않는다.
*/
...
-- 함수는 반드시 반환값이 있으므로 반환타입을 명시해야한다(필수)
    ...
    number
is
    -- 반환값으로 사용한 변수 선언 (선택: 필요없다면 생략가능하다)
    sumNum number;
begin
    sumNum := 0;
    
    -- for loop문으로 숫자사이에 합을 계산한 후 반환한다.
    for i in num1 .. num2 loop
        -- 증가하는 변수i를 sumNum에 누적해서 더해준다.
        sumNum := sumNum + i;
    end loop;
...
-- 실행방법2: 바인드변수를 통한 실행 명령으로 주로 디버깅용으로 사용한다.
...
/*
퀴즈) 주민번호를 전달받아서 성별을 판단하는 함수을 정의하시오.
990909-1000000 => 남자
990919-2000000 => 여자
...
-- 텡이블의 스키마(구조)만 복사한다. where 절에 거짓의 주면 레코드는
-- select되지 않는다.
...
create trigger trig_dept_backup
    ...
    INSERT -- 이벤트 : insert/update/delete와 같은 쿼리실행시 발생된다.
    on trigger_dept_original -- 트리걸를 적용한 테이블명
    for each row 
    /*
        행 단위 트리거 정의한다. 즉 하나의 행이 변화할때마다 트리거가 
        수행된다. 만약 문장(테이블)단위 트리거로 정의하고 싶다면 해당
        문장을 제거하면된다. 이때는 쿼리를 한번 실행할때 트리거도 단
        한번만 실행된다.
    */
begin
    ...
    if Inserting then
        ...
        이와 같은 임시테이블은 행 단위 트리거에서만 사용할 수 있다.
        */
...
insert into trigger_dept_original values(102, '잔산팀', 20, 100);
...
create or replace trigger trig_dept_delete
    /*
    trigger_dept_original에서 레코드를 삭제한 후 행 단위로 해당 트리거를
    실행한다.
    */
    after 
    delete 
...
/
-- 아래와 같이 레코드를 삭제하면 트리거가 자동으로 호출된다.
...
/*
for each row 옵션에 따른 트리커 실행횟수 테스트
...
-- 원본테이블에서 5개의 레코드가 업데이트되믈로 백업테이블에도 5개가
