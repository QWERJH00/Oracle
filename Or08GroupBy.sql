...
select
 ...
 select
    sum(salary) "급여합계",
    to_char(sum(Salary), '999,000') "세자리 컴마",
    ltrim(to_char(sum(salary), 'L999,000')) "좌측공백제거",
    ltrim(to_char(sum(salary), '$999,000')) "통화기호사용"
 from employees where department_id = 10;

-- sum()과 같은 그룹함수는 number타입인 컬럼에서만 사용할 수 있다.
sleect sum(first_name) from employees; -- 에러발생

/*
count() : 그룹화된 레코드의 개술를 카운트할때 사용하는 함수
*/
select count(*) from employees;
select count(employee_id) from employees;

/*
    count() 함수를 사용할 때 위 두가지 방법 모두 가능하지만 *를
...
select 
    ...
    count(distinct job_id) "순수담당업무 개수"
    from employees;
...
select
    department_id "부서 번호",
    rtrim(to_char(sum(salary), '999,999')) "급여합계",
    count(*) "사원수",
...
/*
앞에서 사용했던 쿼리문을 아래와 같이 수정하면 에러가 발생
group by 절에서 사용한 컬럼은 select절에서 사용할 수 있으나
그 외에 단일 컬럼은 select절에서 사용할 수 없다.
...
/*
시나리오) 부서 아이디가 50인 사원들의 직원총합, 평균급여, 급여총합이
...
group by절에 2개의 컬럼을 명시해야한다. 즉 부서와 그룹화 한 후
다시 담당업무별로 그룹화한다.
*/
select
...
where절에 쓰면 에러가 발생. 이런 경우에는 having절에 조건을 추가해야 한다.
...
퀴즈)담당업무별 사원의 최저급여를 출력하시오.
    단, 관리자(Manager)가 없는 사원과 최저급여가 3000미만인 그룹은
...
/*
    문제에서는 급여의 내림차순을 정렬하는 지시사항이 있으나
    현재 select되는 항목이 급여의 최소값이므로 doder by 절에는
    min(salary)를 사용해야한다.
*/

/*
1.전체 사원의 급여 최고액, 최저액, 평균급여를 출력하시오. 컬럼의 별칭은
